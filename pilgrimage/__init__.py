...
def main(names: List[str] = typer.Argument(...), cyclic: bool = False):
    ...
    for name in names:
        ...
        #    raise KeyError(f"Ambiguous resolution of {name}: {' / '.join(l['formatted_address'] for l in geolocate_result)}")
        locations.append(geolocate_result[0])

    locations = sorted(locations, key=lambda loc: pgh.encode(loc['geometry']['location']['lat'], loc['geometry']['location']['lng']))
    print("Locations:")
    ...
    routing.SetArcCostEvaluatorOfAllVehicles(routing.RegisterTransitCallback(lambda a, b: distance(manager.IndexToNode(a), manager.IndexToNode(b))))
    ...
    for n, (i, j) in enumerate(zip(route[:-1], route[1:])):
        print(f' {n+1} - {locations[i]["formatted_address"]} -> {locations[j]["formatted_address"]} ({distance(i, j)/1000:.1f}km)')
